...
async def classify(
    request: Request, image: UploadFile = File(..., description="Image to classify")
):
    """
    ...
    try:
        ...
        return Result(
            code=ApiCode.SERVER_ERROR, message=f"Classification failed: {str(e)}"
        )
...
@limiter.limit("10/minute")
async def denoise(
    request: Request, image: UploadFile = File(..., description="Image to denoise")
):
    """
    ...
    try:
        ...
        return Result(code=ApiCode.SERVER_ERROR, message=f"Denoising failed: {str(e)}")


# --- Similarity ---
@router.post(
    "/simimages", response_model=Result[SimilarityResponse], tags=["Similarity"]
)
@limiter.limit("10/minute")
async def find_similar(
    ...
    try:
        ...
        return Result(code=ApiCode.SERVER_ERROR, message=f"Search failed: {str(e)}")
