...
使用方法:
    cd backend/apps/image-processing/denoising
    python train.py
"""
...
def main():
    ...
    for epoch in tqdm(range(EPOCHS), desc="训练进度"):
        ...
        if val_loss < min_val_loss:
            print("验证损失减小，保存模型...")
            DENOISER_MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
            torch.save(model.state_dict(), str(DENOISER_MODEL_PATH))
    ...
    print(f"模型已保存至: {DENOISER_MODEL_PATH}")
